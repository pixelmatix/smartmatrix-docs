...
You can [add an SD card reader](https://learn.adafruit.com/smartmatrix-animated-gif-player) to the SmartMatrix Shield, and the level shifting buffers are not required for most panels (if yours is displaying graphics without flicker, you don't need them), so this shield is not obsolete.  The connections to the panel are the same, so Arduino sketches that run on the SmartMatrix SD Shield should run on the SmartMatrix Shield.
...
The Schematic, Board Layout, Gerber Files, and BOM can be found in the [SmartMatrix Github Repo](https://github.com/pixelmatix/SmartMatrix/tree/master/extras/hardware) (files start with "SmartMatrixShield_V1").
